...
- `ng-model`: indica un campo (una variabile nello scope), in cui andare a
 registrare il contenuto del tag cui questa direttiva è appesa;
...
Vediamo con un esempio come poter utilizzare il servizio `$http` all’interno
di un controller e sfruttare i metodi della promise restituita:
...
La chiamata `post` (una insert) è analoga, con la sola dfferenza che va
passato un JSON:
